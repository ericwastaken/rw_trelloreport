...
class ReportFormat {
        ...
        .then(listCards => {
          ...
          for (let card of listCards) {
            ...
            if (
              ...
              listItem['excludeNames'].length > 0 &&
              listItem['excludeNames'].find(excludeString =>
                card['name'].toLowerCase().includes(excludeString.toLowerCase())
              )
            ) {
            ...
            // Figure out if we have an includeNames property in the config
            // if we do, we ONLY include cards that contain the string
            if (
              listItem['includeNames'] &&
              Array.isArray(listItem['includeNames']) &&
              listItem['includeNames'].length > 0 &&
              !listItem['includeNames'].find(includeString =>
                card['name'].toLowerCase().includes(includeString.toLowerCase())
              )
            ) {
              // we do have an includeNames array but this card is excluded because it does not contain any of
              // the "include" strings, so we continue.
              continue;
            }
  ...
  static getOutputForOneQuery(
    ...
    return new Promise((resolve, reject) => {
      ...
      startPromise
        .catch(error => {
          // We could not read the name for the user, so just resolve with "User Not Found"
          return Promise.resolve('Could not find username');
        })
        .then(userFullName => {
          const resultHeaderPrefix =
            conf.board.search_query_header_prefix === '' ||
            conf.board.search_query_header_prefix
              ? conf.board.search_query_header_prefix
              : 'Results for ';
          // Output the Query header (including the full user name if we have one)
          this.reportOutput(
            `**${resultHeaderPrefix}'${searchQuery}'${
