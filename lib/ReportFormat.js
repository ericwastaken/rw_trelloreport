...
class ReportFormat {
   ...
   * Note that several items must be defined in /conf/conf.json:
   * - the card output format in 'card_otput_format' (shared with the list report)
   *
   * @param searchQuery - a string to search for. If it starts with '@', it will be treated as a username
   * @param boardId - the id of the board to search within
   * @return {Promise<any>}
   */
  static printSearchReport(searchQuery, boardId) {
    return new Promise((resolve, reject) => {
      // Output Buffer, which will be an array of strings
      let outputBuffer = [];
      // Set some values
      let cardOutputFormat = '`' + conf.board.card_otput_format_search + '`';
      let messageForEmptyList = '\n*No items matched!*';
      // And set a flag so we can detect that all cards filtered out and still output the "no tutorials" message.
      let atLeastOneCard = false;

      // Do our work
      this.reportOutput(`**Results for '${searchQuery}'**`, outputBuffer);
      TrelloApi.getCardsForQuery(searchQuery, boardId)
        .then(listCards => {
          // Iterate over the cards
          for (let card of listCards) {
            // Flag that we have at least 1
