...
class HtmlOutput {
   ...
   * @param mainScriptPath - the directory where the main script is running. Useful to properly respect relative paths for output.
   * @param programArguments - arguments passed into the main script, so that output can be modified based on them.
   */
  static finalOutput(outputBuffer, conf, mainScriptPath, programArguments) {
    let htmlWrapper = fs
    ...
    let htmlOutputPath =
      (programArguments.outputfilepath
        ? path.resolve(mainScriptPath, programArguments.outputfilepath)
        : null) || conf.board.output.htmlOutputPath;
