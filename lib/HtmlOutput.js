...
class HtmlOutput {
   ...
   */
  static finalOutput(outputBuffer, conf, rootPath) {
    let htmlWrapper = fs
      .readFileSync(rootPath + '/' + conf.board.output.htmlWrapperPath)
      .toString();
    let htmlCss = fs.readFileSync(rootPath + '/' + conf.board.output.htmlCssPath).toString();
    let htmlOutputPath = rootPath + '/' + conf.board.output.htmlOutputPath;
    ...
    if (htmlOutputPath) {
      ...
      console.log(`report was output to '${resolvePath(htmlOutputPath)}'.`);
      if (autoOpenOutput) {
